...
----------------------------------------------------------------------------------------------
For use at DTU Physics                                                    GC Parser 1.6.0 (2021)
...
(+) Running
- 	Make sure that "CGCP.py" is in a folder, and that there is a folder called "additional_stuff" that
	contains "GcFunc.py" and "manual_settings.py".
	- 	Alternatively you can place "additional_stuff" in your python-path, to make these 
		functions available always.

- 	Set the values in "additional_stuff/manual_settings.py", which is a script that is executed from the main 
	script. Most importantly but not exclusively the following:
